...
with st.sidebar:
    ...
    if st.session_state.exam_active:
        ...
        if st.button("⚠️ Abandon Exam", use_container_width=True):
            for k in ["exam_active","exam_complete","exam_results","exam_station","exam_question","exam_feedback","exam_answer_input"]:
                if k == "exam_results": st.session_state[k] = []
                elif k in ["exam_active","exam_complete"]: st.session_state[k] = False
                elif k == "exam_station": st.session_state[k] = 0
...
if not st.session_state.exam_active and not st.session_state.exam_complete:
    ...
    <div style='max-width:650px; margin:3rem auto;'>
        <div class='score-card'>
            ...
            <p style='color:#7B9CBB;'>9 stations · 7 minutes each · 63 minutes total</p>
...
# ── STATE B: active exam ────────────────────────────────────────────────────
    ...
    with col_q:
        ...
        if not st.session_state.exam_feedback:
                ...
                if answer_text:
                    ...
                    with st.spinner("Marking your answer…"):
                        st.session_state.exam_feedback = mark_exam_answer(
                            st.session_state.exam_question, answer_text
                        )
                    st.session_state.exam_result_saved = False
                    st.rerun()
                else:
            ...
            st.markdown(f"<div class='{css}'>{icon_s} {fb['feedback']}</div>", unsafe_allow_html=True)
    ...
    with col_timer:
        ...
        for i in range(9):
            cls = "dot-done" if i < station_idx else ("dot-active" if i == station_idx else "dot-todo")
            dots_html += f"<div class='dot {cls}'>{i+1}</div>"
        dots_html += "</div>"
        st.markdown(dots_html, unsafe_allow_html=True)

        # Pure JS timer — no Python reruns needed
        st.components.v1.html(f"""
        <div id="timer" style="font-size:3rem;font-weight:700;color:white;text-align:center;font-variant-numeric:tabular-nums;"></div>
        <div id="tbar" style="background:#1E3A5F;border-radius:4px;height:8px;margin-top:8px;">
          <div id="tfill" style="background:#2979FF;height:8px;border-radius:4px;width:100%;"></div>
        </div>
        <script>
        var remaining = {remaining};
        var total = {STATION_TIME};
        function tick() {{
            if (remaining <= 0) {{ document.getElementById('timer').innerText = '00:00'; return; }}
            var m = Math.floor(remaining/60), s = remaining%60;
            var txt = (m<10?'0':'')+m+':'+(s<10?'0':'')+s;
            var el = document.getElementById('timer');
            el.innerText = txt;
            el.style.color = remaining < 60 ? '#FF5252' : 'white';
            document.getElementById('tfill').style.width = (remaining/total*100)+'%';
            remaining--;
...
# ── STATE C: scorecard ──────────────────────────────────────────────────────
    ...
    with col1:
        if st.button("📋 Take Another Exam", use_container_width=True, type="primary"):
            ...
            st.rerun()
    with col2:
        if st.button("🎓 Go to Tutor", use_container_width=True):
